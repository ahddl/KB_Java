...
public class AverageTest {
    public static void main(String[] args) {
        ...
        //OptionalDouble에는 필드가 isPresent, value 있음 결과 내용을 넣어줌
        //1. if 문으로 값이 있는지 체크해서 있으면 처리, 없을 때도 처리 내용 써주고,
        if(result.isPresent()) {
            System.out.println("평균은 "+ result.getAsDouble());  //n 입력시, 평균은 3.0 출력
        }else {
            System.out.println("평균을 구할 수 없음");  //n2 입력시, 출력
        }
        ...
        //3. 있을 때만 처리하고 싶은 경우 ifPresent(처리내용)
